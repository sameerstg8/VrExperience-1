...
namespace UnityEditor.Rendering.Universal.PostProcessing {
    ...
    /// </summary>
    [CustomPropertyDrawer(typeof(CinematicURPPostProcessing_PRISM.CustomPostProcessSettings), true)]
    internal class CustomPostProcessSettingsEditor : PropertyDrawer {
        ...
        /// </summary>
        void InitList(ref ReorderableList reorderableList, List<string> elements, string headerName, CustomPostProcessInjectionPoint injectionPoint, CinematicURPPostProcessing_PRISM feature){
        ...
        private void Init(SerializedProperty property){
            ...
            if(!propertyStates.ContainsKey(path)){
                ...
                var feature = property.serializedObject.targetObject as CinematicURPPostProcessing_PRISM;
                ...
                InitList(ref state.listBeforePostProcess, feature.settings.renderersBeforePostProcess, "Before Post Process & PRISM Effects", CustomPostProcessInjectionPoint.BeforePostProcess, feature);
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if(GUILayout.Button("Add all Cinematic Post-Processing Effects"))
            {
                var feature = property.serializedObject.targetObject as CinematicURPPostProcessing_PRISM;
                List<string> elements = feature.settings.renderersBeforePostProcess;
                foreach (var type in _availableRenderers[CustomPostProcessInjectionPoint.BeforePostProcess])
                {
                    if (!elements.Contains(type.AssemblyQualifiedName))
                        {
                            elements.Add(type.AssemblyQualifiedName);
                ...
                List<string> elementsTwo = feature.settings.renderersAfterOpaqueAndSky;
                foreach (var type in _availableRenderers[CustomPostProcessInjectionPoint.AfterOpaqueAndSky])
                {
                    if (!elementsTwo.Contains(type.AssemblyQualifiedName))
                    {
                        elementsTwo.Add(type.AssemblyQualifiedName);
                        forceRecreate(feature); // This is done since OnValidate doesn't get called.
                    };
                }

                List<string> elementsThree = feature.settings.renderersAfterPostProcess;
                foreach (var type in _availableRenderers[CustomPostProcessInjectionPoint.AfterPostProcess])
                {
                    if (!elementsThree.Contains(type.AssemblyQualifiedName))
                    {
                        elementsThree.Add(type.AssemblyQualifiedName);
                        forceRecreate(feature); // This is done since OnValidate doesn't get called.
                    };
                }
            }
        ...
        private void forceRecreate(CinematicURPPostProcessing_PRISM feature){
            feature.Create();
        }
